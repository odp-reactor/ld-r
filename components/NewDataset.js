...
class NewDataset extends React.Component {
    ...
    handleCreateDataset() {
        ...
        if (!this.state.endpointURI) {
            ...
            if (
                endpointURI.indexOf('http://') === -1 &&
                endpointURI.indexOf('https://') === -1
            ) {
    ...
    render() {
            ...
            if (!enableAddingNewDatasets) {
                errorDIV = (
                    <div className="ui warning message">
                        <div className="header">
                            {' '}
                            It is not possible to add new datasets in this
                            application!
                        </div>
                    </div>
                );
                    ...
                    {this.state.resourceFocusType || this.state.endpointURI ? (
                        <div>
                            ...
                            <PrefixBasedInput
                                ...
                                noFocus={true}
                                spec={{ value: '' }}
                                onDataEdit={this.handleResourceFocusTypeChange.bind(
                                    this
                                )}
                                placeholder="Resource Focus Type / or leave it empty for all resource types"
                                onEnterPress={this.handleCreateDataset.bind(
                                    this
                                )}
        ...
        let headerDIV = (
            <h2>Add a new dataset by connecting to a SPARQL endpoint </h2>
        );
        if (enableCSVImport) {
            headerDIV = (
                <h2>
                    Add a new dataset by connecting to a SPARQL endpoint or{' '}
                    <a className="ui big icon button" href="/importCSV">
                        <i className="icon file alternate outline"></i> Import
                        CSV data
                    </a>
                </h2>
            );
