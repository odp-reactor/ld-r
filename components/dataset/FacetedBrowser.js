...
class FacetedBrowser extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            analysisProps: {},
            pivotConstraint: '',
            envState: this.props.FacetedBrowserStore.envState,
            importedEnvState: this.props.FacetedBrowserStore.importedEnvState
        };
    }
    ...
    handlePivotChange(queryConstraints, config) {
        ...
        this.context.executeAction(loadFacets, {
            mode: 'init',
            id: datasetURI,
            page: 1,
            selection: {},
    ...
    selectFacetsAndLoadEmptyFacets(env) {
        ...
        zeroLengthSelection.forEach(function(el) {
            self.context.executeAction(loadFacets, {
                mode: 'sideEffect',
                id: env.id,
                page: env.page,
                selection: {
    ...
    createFConfig(datasetURI) {
        this.context.executeAction(createASampleFacetsConfig, {
            dataset: datasetURI,
            redirect: 1
        });
    }
    ...
    getNecessaryFaccetsConfig() {
        let facetConfigs = {};
        let cnf = JSON.parse(
            JSON.stringify(this.props.FacetedBrowserStore.config)
        );
        ...
        for (let prop in cnf) {
            ...
            if (cnf[prop].restrictAnalysisToSelected) {
                if (!facetConfigs[prop]) {
                    facetConfigs[prop] = {
                        restrictAnalysisToSelected:
                            cnf[prop].restrictAnalysisToSelected
                    };
                } else {
                    facetConfigs[prop].restrictAnalysisToSelected =
                        cnf[prop].restrictAnalysisToSelected;
                }
            }
                    ...
                    if (!propConfig.isHidden) {
                        properties.push({
                            label: propConfig
                                ? propConfig.label
                                    ? propConfig.label
                                    : self.getPropertyLabel(el)
                                : self.getPropertyLabel(el),
                            value: el,
                            valueType: 'uri',
                            position:
                                propConfig && propConfig.position
                                    ? propConfig.position
                                    : 0,
                            category:
                                propConfig && propConfig.category
                                    ? propConfig.category
                                    : propConfig && propConfig.pivotDataset
                                        ? ['Linked Entities']
                                        : ''
                        });
                    }
    ...
    handleShowMoreFacet(propertyURI, fpage) {
        ...
        this.context.executeAction(loadFacets, {
            ...
            selection: {
                ...
                status: 1,
                prevSelection: this.state.selection,
                options: { invert: this.state.invert, range: this.state.range }
            },
            fpage: fpage
        });
    }
    ...
    handleToggleRange(propertyURI, rangeObj) {
        ...
        if (!this.state.range[propertyURI]) {
            ...
            this.state.selection[propertyURI] = [
                { value: 'ldr:range', valueType: 'literal', dataType: '' }
            ];
        } else {
    ...
    handleOnCheck(level, valueType, dataType, status, value, propertyURI) {
        ...
        if (level === 2) {
            ...
            if (status) {
                ...
                if (Array.isArray(value)) {
                    value.forEach(item => {
                        this.state.selection[propertyURI].splice(
            ...
            for (let key in this.state.selection) {
                ...
                if (
                    !this.state.selection[key].length &&
            ...
            if (
                ...
                this.state.selection[value].length
            ) {
                hadAnySelected = 1;
            } else {
                hadAnySelected = 0;
    ...
    render() {
                    ...
                    <div className="ui grid">
                        <div className="ui column">
                            <div className="ui warning message">
                                <h2>{this.props.FacetedBrowserStore.error}</h2>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        let showFactes = 0;
        let configDiv = '';
        let properties = this.buildMasterFacet(
            this.props.FacetedBrowserStore.datasetURI
        );
        ...
        if (this.props.FacetedBrowserStore.datasetURI) {
            let list = properties.map(function(node, index) {
                //console.log(self.props.FacetedBrowserStore.facets);
                if (
                    self.props.FacetedBrowserStore.facets[node.value] &&
                    self.props.FacetedBrowserStore.facets[node.value].length
                ) {
                    showFactes = 1;
                    ...
                    if (self.state.expandedFacet) {
                        if (self.state.expandedFacet === node.value) {
                            return (
                                <Facet
                                    ...
                                    minHeight={550}
                                    maxHeight={700}
                                    onCheck={self.handleOnCheck.bind(
                                        self,
                                        2,
                                        self.props.FacetedBrowserStore.facets[
                                            node.value
                                        ][0].valueType,
                                        self.props.FacetedBrowserStore.facets[
                                            node.value
                                    ...
                                    onInvert={self.handleToggleInvert.bind(
                                        self,
                                        node.value
                                    )}
                                    onAnalyzeProp={self.handleAnalysisProps.bind(
                                        self,
                                        node.value
                                    )}
                                    onRange={self.handleToggleRange.bind(
                                        self,
                                        node.value
                                    )}
                                    onShowMore={self.handleShowMoreFacet.bind(
                                        self,
                                        node.value
                                    )}
                                    key={self.findIndexInProperties(
                                        properties,
                                        node.value
                                    )}
                                    spec={{
                                        propertyURI: node.value,
                                        property: self.getPropertyLabel(
                                            node.value
                                        ),
                                        instances:
                                            self.props.FacetedBrowserStore
                                                .facets[node.value],
                                        total:
                                            self.props.FacetedBrowserStore
                                                .facetsCount[node.value],
                                        query:
                                            self.props.FacetedBrowserStore
                                                .facetQuery[node.value],
                                        queryConstraints:
                                            self.props.FacetedBrowserStore
                                                .facetQueryConstraints[
                                                    node.value
                                                ]
                                    }}
                                    config={self.getPropertyConfig(
                                        self.props.FacetedBrowserStore
                                            .datasetURI,
                                        node.value
                                    )}
                                    ...
                                    toggleExpandFacet={self.toggleExpandFacet.bind(
                                        self
                                    )}
                                    onPivotChange={self.handlePivotChange.bind(
                                        self,
                                        self.props.FacetedBrowserStore
                                            .facetQueryConstraints[node.value],
                                        self.getPropertyConfig(
                                            self.props.FacetedBrowserStore
                                                .datasetURI,
                                            node.value
                                        )
                        ...
                        return (
                            <Facet
                                ...
                                config={self.getPropertyConfig(
                                    self.props.FacetedBrowserStore.datasetURI,
                                    node.value
                                )}
                                datasetURI={
                                    self.props.FacetedBrowserStore.datasetURI
            ...
            if (this.state.hideFirstCol) {
                facetsDIV = showFactes ? (
                    <div className="ui stackable nine wide column">{list}</div>
                ) : (
                    ''
                );
            } else {
                facetsDIV = showFactes ? (
                    <div className="ui stackable five wide column">{list}</div>
                ) : (
                    ''
                );
            }
            ...
            if (dcnf.allowInlineConfig) {
                configDiv = (
                    <a
                        onClick={this.createFConfig.bind(
                            this,
                            storeObj.datasetURI
                        )}
            ...
            return (
                ...
                >
                    ...
                    this.state.envState[this.state.envState.length - 1].desc ? (
                    ...
                    <div className="ui vertically padded stackable grid">
                        {this.state.hideFirstCol ? (
                            ...
                            <div className="ui stackable four wide column">
                                <Facet
                                    ...
                                    analysisProps={self.state.analysisProps}
                                    range={self.state.range}
                                    onCheck={this.handleOnCheck.bind(
                                        this,
                                        1,
                                        'uri',
                                        ''
                                    ...
                                    config={{ label: 'Selected Properties' }}
                                    datasetURI={
                                        this.props.FacetedBrowserStore
                                            .datasetURI
                                    }
                                />
                                {configDiv}
                        ...
                        <div
                            className={
                                'ui stackable ' + resSize + ' wide column'
                            }
                        >
                            <DatasetFB
                                ...
                                pagerSize={pagerSize}
                                currentPage={
                                    this.props.FacetedBrowserStore.page
                                }
                                resources={
                                    this.props.FacetedBrowserStore.resources
                                }
                                datasetURI={
                                    this.props.FacetedBrowserStore.datasetURI
                                }
                                ...
                                onExpandCollapse={this.toggleResourceCol.bind(
                                    this
                                )}
                                onShowAllResources={this.toggleShowAllResources.bind(
                                    this
                                )}
                                handleClick={this.gotoPage.bind(this)}
                                handleBackToPrevPivotState={this.handleBackToPrevPivotState.bind(
                                    this
                                )}
                            />
                        </div>
                        ...
                        <div className="ui column">
                            {this.state.pivotConstraint ? (
                                ...
                                <div className="ui segment">
                                    <h2>
                                        List of available datasets to browse
                                    </h2>
...
FacetedBrowser = connectToStores(
    FacetedBrowser,
    [FacetedBrowserStore],
    function(context, props) {
        return {
            FacetedBrowserStore: context
                .getStore(FacetedBrowserStore)
                .getState()
        };
    }
);
