...
class DatasetPager extends React.Component {
    ...
    buildLink(page, color, icon) {
        if (this.props.handleClick) {
            return (
                ...
                >
                    {' '}
                    {icon ? <i className={icon}></i> : <span>{page}</span>}{' '}
                </a>
            );
        } else {
            return (
    ...
    saveEnvState(desc) {
        if (desc.trim()) {
            ...
            this.context.executeAction(saveFacetsEnvState, {
                ...
                selection: selection,
                pivotConstraint: this.props.pivotConstraint,
                searchTerm: this.state.searchMode
    ...
    render() {
        ...
        if (this.props.noOfAnalysisProps && this.props.noOfAnalysisProps > 1) {
            ...
            if (
                this.props.noOfAnalysisProps &&
                this.props.noOfAnalysisProps === 1
            ) {
                v_options = [
                    {
                        key: 1,
        ...
        let iconC =
            this.state.config && this.state.config.datasetViewer
                ? v_icons[this.state.config.datasetViewer]
                    ? v_icons[this.state.config.datasetViewer]
                    : defaultViewIcon
                : defaultViewIcon;
        const v_trigger = (
        ...
        if (
            this.props.total ||
            (this.props.hasResources && !this.props.total)
        ) {
            ...
            if (totalPages > threshold) {
                ...
                for (
                    i = startI;
                    i <= currentPage + threshold && i <= totalPages;
                    i++
                ) {
                    if (i === currentPage) {
                ...
                //last page
                pageList.push(
                    self.buildLink(totalPages, 'grey', 'step forward icon')
                );
            } else {
                        ...
                        <div className="item">
                            {this.props.showAllResources ? (
                                ...
                                <span>
                                    <span
                                        onDoubleClick={
                                            this.props.handleToggleShowQuery
                                        }
                                    >
                                        {totalPages}
                                    </span>{' '}
