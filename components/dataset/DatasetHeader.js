...
class DatasetHeader extends React.Component {
    ...
    render() {
        ...
        if (resourceFocusType) {
            if (
                !resourceFocusType.length ||
                (resourceFocusType.length && !resourceFocusType[0])
            ) {
        ...
        if (dcnf.constraint) {
            for (let prop in dcnf.constraint) {
                constraints.push(
                    URIUtil.getURILabel(prop) +
                        ': ' +
                        dcnf.constraint[prop].join(',')
                );
            }
        ...
        if (this.props.datasetURI) {
            datasetTitle = (
                <a href={this.props.datasetURI}> {this.props.datasetURI} </a>
            );
            if (dcnf && dcnf.datasetLabel) {
                datasetTitle = (
                    <a href={this.props.datasetURI}> {dcnf.datasetLabel} </a>
                );
            }
        }
        return (
                ...
                <h3 className="ui header">
                    {this.props.total ? (
                        <a
                            target="_blank"
                            href={
                                '/exportQuery/NTriples/' +
                                encodeURIComponent(this.props.datasetURI) +
                                '/' +
                                encodeURIComponent(this.props.resourceQuery)
                            }
                        >
                            <span className="ui big blue circular label">
                                {this.props.searchMode
                                    ? this.addCommas(
                                        this.props.resourcesLength
                                    ) + '/'
                                    : ''}
                                {this.addCommas(this.props.total)}
                            </span>
                        </a>
                    ) : this.props.hasResources ? (
                        <span
                    ...
                    Resources of type {typeSt} in{' '}
                    {datasetTitle ? datasetTitle : ' all local datasets'}{' '}
                    {dcnf.constraint ? (
                        <span>
                            <Popup
                                trigger={
                                    <i className="ui orange filter icon link ">
                                        {' '}
                                    </i>
                                }
                    ...
                    {this.props.pivotConstraint &&
                    this.props.pivotConstraint.trim() ? (
                            <span onClick={this.props.handleBackToPrevPivotState}>
                                <Popup
                                    trigger={
                                        <div className="ui labeled mini button">
                                            <div className="ui mini blue button">
                                                {' '}
                                                <i className="ui zoom out icon">
                                                    {' '}
                                                </i>{' '}
                                            Zoom out
                                            </div>
                                            <a className="ui basic left pointing label">
                                                <i className="ui violet filter icon link ">
                                                    {' '}
                                                </i>
                                            </a>
                                        </div>
