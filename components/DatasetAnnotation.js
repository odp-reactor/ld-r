...
class DatasetAnnotation extends React.Component {
    ...
    componentDidUpdate() {
        if (this.state.annotationMode && !this.state.hideFeedback) {
            let tags = this.prepareTagsForCloud(
                this.props.DatasetAnnotationStore.tags
            );
            if (tags.length) {
    ...
    prepareTagsForCloud(obj) {
        ...
        for (let prop in obj) {
            tags.push({
                ...
                text: obj[prop].text,
                html: { title: obj[prop].count, target: '_blank' }
            });
        }
        return tags;
    }
    ...
    startInterval() {
        ...
        let intervalId = setInterval(() => {
            self.context.executeAction(countAnnotatedResourcesWithProp, {
                id: self.state.storeInNewDataset
                    ? self.state.storingDataset
                    : self.state.datasetURI,
            ...
            if (
                self.props.DatasetAnnotationStore.stats.annotated &&
                self.props.DatasetAnnotationStore.stats.annotated ===
                    self.props.DatasetAnnotationStore.stats.total
            ) {
    ...
    render() {
            ...
            if (!enableDatasetAnnotation) {
                errorDIV = (
                    <div className="ui warning message">
                        <div className="header">
                            {' '}
                            It is not possible to annotate datasets in this
                            application!
                        </div>
                    </div>
                );
        ...
        */
        if (!errorDIV) {
            formDIV = (
                <Form size="big">
                    ...
                    >
                        <i className="blue icon setting"></i> Advanced Options
                    </a>
                        ...
                        <div className="ui list">
                            ...
                            <div className="item">
                                ...
                                <input
                                    ...
                                    onChange={this.handleConfidenceChange.bind(
                                        this
                                    )}
                            ...
                            <div className="item">
                                ...
                                <input
                                    ...
                                    onChange={this.handleStopWordsChange.bind(
                                        this
                                    )}
                            ...
                            <div className="item">
                                ...
                                <input
                                    ...
                                    onChange={this.handleBatchSizeChange.bind(
                                        this
                                    )}
                            ...
                            <div className="item">
                                ...
                                <input
                                    ...
                                    onChange={this.handleFeedbackIntervalChange.bind(
                                        this
                                    )}
                            ...
                            <Form.Group>
                                ...
                                <Form.Radio
                                    ...
                                    onChange={this.handleFeedbackCheckBox.bind(
                                        this
                                    )}
                                />
                                <Form.Radio
        ...
        if (this.state.annotationMode) {
            let remainingTime = 0;
            if (
                this.props.DatasetAnnotationStore.stats.prevAnnotated &&
                this.props.DatasetAnnotationStore.stats.annotated &&
                this.props.DatasetAnnotationStore.stats.total
            ) {
                remainingTime = Math.floor(
                    (this.state.feedbackInterval *
                        (this.props.DatasetAnnotationStore.stats.total -
                            this.props.DatasetAnnotationStore.stats
                                .annotated)) /
                        (this.props.DatasetAnnotationStore.stats.annotated -
                            this.props.DatasetAnnotationStore.stats
                                .prevAnnotated)
            ...
            if (remainingTime) {
                if (remainingTime >= 60) {
                    if (remainingTime >= 3600) {
                        remainingTimeDIV = (
                            <span>
                                ...
                                <b>
                                    {Math.round((remainingTime / 3600) * 100) /
                                        100}
                                </b>{' '}
                                hour(s)
                            </span>
                        ...
                        remainingTimeDIV = (
                            <span>
                                Estimated Remaining Time: ~{' '}
                                <b>
                                    {Math.round((remainingTime / 60) * 100) /
                                        100}
                                </b>{' '}
                                minute(s)
                            </span>
                    ...
                    remainingTimeDIV = (
                        <span>
                            Estimated Remaining Time: ~ <b>{remainingTime}</b>{' '}
                            second(s)
                        </span>
                    );
                }
            ...
            progressDIV = (
                <div>
                    <div className="ui list">
                            ...
                            <b>
                                <a
                                    href={
                                        '/dataset/1/' +
                                        encodeURIComponent(
                                            this.state.datasetURI
                                        )
                                    }
                                    target="_blank"
                                >
                                    {this.findDatasetLabel(
                                        this.state.datasetURI
                                    )}
                                </a>
                            </b>{' '}
                            {!this.state.storingDataset ? (
                                ''
                            ) : (
                                <span>
                                    {' '}
                        ...
                        <div className="item">
                            Property used: <b>{this.state.propertyURI}</b>
                        </div>
                    </div>
                    {this.props.DatasetAnnotationStore.stats.annotated &&
                    this.props.DatasetAnnotationStore.stats.annotated ===
                            ...
                            <div>
                                <Progress
                                    percent={
                                        this.props.DatasetAnnotationStore.stats
                                            .annotated
                                            ? Math.floor(
                                                (this.props.DatasetAnnotationStore
                                                    .stats.annotated /
                                                  this.props
                                ...
                                items{' '}
                                    <a
                                        ...
                                        onClick={this.handleAnnotateDataset.bind(
                                            this
                                        )}
                                    >
                                        <i className="ui icon blue refresh"></i>{' '}
                                    refresh
                                    </a>
                                </Progress>
...
DatasetAnnotation = connectToStores(
    DatasetAnnotation,
    [DatasetsStore, DatasetAnnotationStore],
    function(context, props) {
        return {
            DatasetsStore: context.getStore(DatasetsStore).getState(),
            DatasetAnnotationStore: context
                .getStore(DatasetAnnotationStore)
                .getState()
        };
    }
);
