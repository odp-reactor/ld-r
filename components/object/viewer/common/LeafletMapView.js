        ...
        if (process.env.BROWSER) {
            ...
            if (self.props.geometry && self.props.geometry.length) {
                ...
                self.props.geometry.forEach((geo, index) => {
                    ...
                    if (!style) {
                        style = {
                            ...
                            opacity: 1,
                            weight: 1.5,
                            fillColor: weights[index]
                                ? colorMap[weights[index]]
                ...
                polygonsDIV = polygons.map((polygon, index) => {
                    return (
                        <Polygon
                            ...
                            key={index}
                            positions={self.reversePolygonCoords(
                                polygon.coords
                            )}
                        >
                            {polygon.hint ? (
                ...
                multipolygonsDIV = multipolygons.map((mpolygon, index) => {
                    return (
                        <Polygon
                            ...
                            key={index}
                            positions={self.reverseMultiPolygonCoords(
                                mpolygon.coords
                            )}
                        >
                            {mpolygon.hint ? (
                ...
                if (features.length) {
                    ...
                    geoJSONDIV = (
                        <GeoJSON data={geojson} style={self.styleGeoJSON} />
                    );
                }
            }
            let minHeight = 210;
