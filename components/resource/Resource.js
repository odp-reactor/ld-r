...
class Resource extends React.Component {
    ...
    render() {
        ...
        let list = this.props.properties.map(function(node, index) {
            ...
            if (!node.config || !node.config.isHidden) {
                ...
                if (
                    ...
                    node.propertyURI ===
                    'https://github.com/ali1k/ld-reactor/blob/master/vocabulary/index.ttl#annotatedBy'
                ) {
                    annotationMetaDIV = (
                        <PropertyReactor
                            key={index}
                    ...
                    node.propertyURI ===
                    'https://github.com/ali1k/ld-reactor/blob/master/vocabulary/index.ttl#annotations'
                ) {
                    annotationDIV = (
                        <PropertyReactor
                            key={index}
        ...
        if (
            self.props.config &&
            typeof self.props.config.templateResource !== 'undefined' &&
            self.props.config.templateResource[0] === self.props.resource
        ) {
            disableDelete = 1;
        }
        let deleteable = 0;
        if (
            self.props.config &&
            !this.props.readOnly &&
            typeof self.props.config.allowResourceDelete !== 'undefined' &&
            parseInt(self.props.config.allowResourceDelete)
        ) {
            deleteable = 1;
        }
        return (
                    ...
                    <div className="ui column">
                        ...
                        <h2>
                            ...
                            {deleteable ? (
                                <a
                                    className={
                                        'medium ui circular basic icon button' +
                                        (disableDelete ? ' disabled' : '')
                                    }
                                    onClick={this.handleDeleteResource.bind(
                                        this,
                                        this.props.datasetURI,
                                        decodeURIComponent(this.props.resource)
                                    )}
                                    title={
                                        disableDelete
                                            ? 'can not delete this resource because it is set as a template resource.'
