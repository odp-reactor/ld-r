    ...
    render() {
        ...
        let list = this.props.properties.map(function(node, index) {
            ...
            if (!node.config || !node.config.isHidden) {
                ...
                if (
                    node.propertyURI === 'http://xmlns.com/foaf/0.1/depiction'
                ) {
                    depiction = node.instances[0].value;
                }
                if (
                    node.propertyURI === 'http://xmlns.com/foaf/0.1/thumbnail'
                ) {
                ...
                if (
                    node.propertyURI === 'http://www.w3.org/2006/vcard/ns#email'
                ) {
                    email = node.instances[0].value;
                }
                if (
                    node.propertyURI === 'http://xmlns.com/foaf/0.1/firstName'
                ) {
                ...
                if (
                    node.propertyURI === 'http://dbpedia.org/ontology/birthDate'
                ) {
                    obirthDate = node.instances[0].value;
                }
                if (
                    node.propertyURI === 'http://dbpedia.org/property/birthDate'
                ) {
                ...
                if (
                    node.propertyURI === 'http://dbpedia.org/ontology/deathDate'
                ) {
                    odeathDate = node.instances[0].value;
                }
                if (
                    node.propertyURI === 'http://dbpedia.org/property/deathDate'
                ) {
                ...
                if (
                    node.propertyURI === 'http://dbpedia.org/property/children'
                ) {
                ...
                if (
                    node.propertyURI === 'http://dbpedia.org/ontology/knownFor'
                ) {
        ...
        if (spouse) {
            spouseDIV = spouse.map((node, index) => {
                return (
                    ...
                    >
                        {URIUtil.getURILabel(node.value)}
                    </a>
                );
            });
        }
                    ...
                    <div className="ui column">
                            ...
                            <div className="twelve wide column">
                                <div className="ui huge divided list">
                                    {birthDate ? (
                                        <div className="item">
                                            <i className="icons">
                                                <i className="ui icon circle thin"></i>
                                            </i>{' '}
                                            {birthDate}{' '}
                                            {birthPlace ? (
                                                <a
                                    ...
                                    {spouse ? (
                                        <div className="item ui">
                                            <i className="icons">
                                                {' '}
                                                <i className="ui icon  blue male"></i>
                                            </i>
                                            <i className="icons">
                                                {' '}
                                                <i className="ui icon pink female"></i>
                                            </i>{' '}
                                            {spouseDIV}
                                        </div>
                                    ) : (
                                    ...
                                    {children ? (
                                        <div className="item ui">
                                            <i className="icons">
                                                <i className="ui icon green child"></i>
                                            </i>{' '}
                                            {childrenDIV}
                                        </div>
                                    ) : (
                                    ...
                                    {aboutP ? (
                                        <div className="item"> {aboutP}</div>
                                    ) : (
                                        ''
                                    )}
                                    {homepage ? (
                                        <a className="item" href={homepage}>
                                            {' '}
                                            <i className="icons">
                                                <i className="ui icon violet home"></i>
                                            </i>{' '}
                                            {homepage}
                                        </a>
                                    ) : (
                                    ...
                                    {email ? (
                                        ...
                                        >
                                            {' '}
                                            <i className="icons">
                                                <i className="ui icon blue mail outline"></i>
                                            </i>{' '}
                                            {email}
                                        </a>
                                    ) : (
                        ...
                        >
                            {!this.state.showDetails ? (
                                <span>
                                    <i className="ui toggle down icon"></i>show
                                    details...
                                </span>
                            ) : (
                                <span>
                                    <i className="ui toggle up icon"></i>hide
                                    details...
                                </span>
                            )}
                        </div>
                        <div className={detailClasses}>{mainDIV}</div>
                    </div>
                </div>
            </div>
        );
