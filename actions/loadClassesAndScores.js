export default function loadClassesAndScores(context, payload, done) {
    context.dispatch('LOADING_DATA', {}); // loaderOn (in ApplicationStore)
    context.service.read(
        'class.getClassesAndScores',
        payload,
        { timeout: 25 * 1000 },
        function(err, res) {
            if (err) {
                ...
                context.dispatch('CLEAN_CLASSES_AND_SCORES_SUCCESS', res);
                context.dispatch('CLEAN_GMAP_SUCCESS', res);
                context.dispatch('LOAD_CLASSES_AND_SCORES_SUCCESS', res); // OK
            }
