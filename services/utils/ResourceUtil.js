...
class ResourceUtil {
    ...
    parseProperties(
        ...
        //resource config
        configurator.prepareResourceConfig(
            user,
            1,
            datasetURI,
            resourceURI,
            ...
            rconfig => {
                ...
                if (propertyPath && propertyPath.length) {
                    asyncTasks.push(function(callback) {
                        ...
                        configurator.preparePropertyConfig(
                            user,
                            1,
                            datasetURI,
                            resourceURI,
                ...
                if (
                    rconfig &&
                    rconfig.resourceLabelProperty &&
                    rconfig.resourceLabelProperty.length
                ) {
                    searchForTitle = true;
                    newTitel = '';
                }
                for (let propURI in rpIndex.propIndex) {
                    ...
                    if (searchForTitle) {
                        if (
                            rconfig.resourceLabelProperty.indexOf(propURI) !==
                            -1
                        ) {
                            let tmpArr = [];
                            ...
                            dconfig => {
                                ...
                                if (filterByCategory) {
                                    if (
                                        !dconfig ||
                                        !dconfig.category ||
                                        category !== dconfig.category[0]
                                    ) {
                                        //skip property
                                        callback2();
                                    } else {
                                        let property = self.getPropertyLabel(
                                            propURI
                                        );
                                        output.push({
                                            propertyURI: propURI,
                ...
                let userIsEditor = 0,
                    checkEditorship;
                if (user) {
                    checkEditorship = checkEditAccess(
                        user,
                        datasetURI,
                        resourceURI,
                        resourceType,
                    ...
                    if (
                        checkEditorship.access &&
                        checkEditorship.type === 'full'
                    ) {
                        userIsEditor = 1;
                    }
                }
                    ...
                    output.forEach(function(el) {
                        ...
                        if (user) {
                            if (
                                ...
                                user.id === el.instances[0].value
                            ) {
                                userIsCreator = 1;
                            }
                            accessLevel = checkEditAccess(
                                user,
                                datasetURI,
                                resourceURI,
                                resourceType,
    ...
    buildResourcePropertyIndex(bindings) {
        ...
        bindings.forEach(function(el) {
            ...
            if (el.p.value === 'http://purl.org/dc/terms/title') {
                ...
                el.p.value === 'http://www.w3.org/2000/01/rdf-schema#label'
            ) {
                output.title = el.o.value;
            } else if (
                el.p.value === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type'
            ) {
            ...
            if (propIndex[el.p.value]) {
                propIndex[el.p.value].push({
                    ...
                    valueType: el.o.type,
                    dataType:
                        el.o.type === 'typed-literal' ? el.o.datatype : '',
                        ...
                        asyncTasks.push(function(callback) {
                            configurator.preparePropertyConfig(
                                ...
                                dconfig => {
                                    ...
                                    if (
                                        configExceptional &&
                                        configExceptional.extensions
                                    ) {
                                        configExceptional.extensions.forEach(
                                            function(ex) {
                                                if (
                                                    ex.spec.propertyURI ===
                                                    el.p.value
                                    ...
                                    if (propIndex[el.p.value]) {
                                        propIndex[el.p.value].push({
                                            ...
                                            lang: el.o['xml:lang']
                                                ...
                                                : '',
                                            extended: parseInt(
                                                el.hasExtendedValue.value
                                            ),
                                            valueLabel: el.oLabel
                                                ? el.oLabel.value
                                                : '',
                                            valueTitle: el.oTitle
                                                ? el.oTitle.value
                                                : ''
                                        });
                                    } else {
    ...
    deleteAdminProperties(list) {
        let out = [];
        const adminProps = [
            'https://github.com/ali1k/ld-reactor/blob/master/vocabulary/index.ttl#isSuperUser',
            'https://github.com/ali1k/ld-reactor/blob/master/vocabulary/index.ttl#isActive',
